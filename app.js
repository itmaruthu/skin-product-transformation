...
class SkincareApp {
  ...
  bindDOMEvents() {
    ...
    // Animation Variation Selectors
    const animSelectors = document.querySelectorAll('.btn-anim-select');
    const layoutContainer = document.getElementById('testimonials-layout');
    const testimonialsSection = document.getElementById('testimonials-section');

    animSelectors.forEach(btn => {
      btn.addEventListener('click', () => {
        if (btn.classList.contains('active')) return;

        // Update active button
        document.querySelector('.btn-anim-select.active')?.classList.remove('active');
        btn.classList.add('active');

        // Update layout variation class
        const variant = btn.getAttribute('data-var');
        if (layoutContainer) {
          layoutContainer.className = 'testimonials-layout var-' + variant;
        }

        // Instantly re-trigger transition for preview
        if (testimonialsSection && testimonialsSection.classList.contains('active')) {
          testimonialsSection.classList.remove('active');
          void testimonialsSection.offsetWidth; // Force layout reflow
              ...
              <div class="product-info">
                ...
                <div class="product-rating-row">
                  ...
                  <button class="btn-card-add" onclick="window.App.addProductToCart('${prod.id}')">
                    <svg class="cart-icon" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
